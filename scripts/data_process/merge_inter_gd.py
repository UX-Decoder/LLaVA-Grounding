...
instruct="/comp_robot/zhanghao/datasets/llava/merged_v3_fixed.json"
instruct_out="/comp_robot/zhanghao/datasets/llava/merged_v3_fixed_inter.json"
            ...
            if max_score>0.6:
                ...
                for ii,(q_res,q_new) in enumerate(zip(convs_res[::2],anno['new_qs'])):
                    ...
                    if q_new is not None:
                        if 'with grounding' in q_res['value']:
                            q_res['value']=pre_str+q_new+' (with grounding)'
                        else:
