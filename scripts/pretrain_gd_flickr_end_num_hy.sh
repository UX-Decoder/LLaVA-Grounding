...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/lihongyang/model/llava_stage1_new_gd_flickr_end_num/
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed llava/train/train_hao_seg_flickr.py \
    ...
    --whole_model /comp_robot/zhanghao/model/llava_stage1_new_gd_flickr_end/ \
    --load_vision True \
    ...
    --opt \
    flickr.TEST.BATCH_SIZE_TOTAL=32,flickr.TRAIN.BATCH_SIZE_TOTAL=32,flickr.gd_mode=end_num,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
