...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_ev/
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed --include=localhost:1,2,4,7 --master_port 29503 llava/train/train_hao_seg_flickr.py \
    --deepspeed scripts/zero2.json \
    --model_name_or_path /comp_robot/zhanghao/ckpts/vicuna/vicuna-7b-v1.3/ \
    --whole_model /comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_filtered/checkpoint-1000/ \
    --load_vision True \
    ...
    --report_to wandb \
    --config_file \
    configs/openseed/openseed_swint_lang_refcoco_val_g.yaml\
    --opt \
    flickr.TEST.BATCH_SIZE_TOTAL=64,flickr.TRAIN.BATCH_SIZE_TOTAL=64,COCO.TEST.BATCH_SIZE_TOTAL=16,COCO.TRAIN.BATCH_SIZE_TOTAL=16,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log_ev 2>&1
