...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_fixed_refonly_100q/
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed --master_port $port llava/train/train_hao_seg_flickr.py \
    ...
    --opt \
    MODEL.DECODER.NUM_OBJECT_QUERIES=100,flickr.TEST.BATCH_SIZE_TOTAL=32,flickr.TRAIN.BATCH_SIZE_TOTAL=32,COCO.TEST.BATCH_SIZE_TOTAL=32,COCO.TRAIN.BATCH_SIZE_TOTAL=32,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
