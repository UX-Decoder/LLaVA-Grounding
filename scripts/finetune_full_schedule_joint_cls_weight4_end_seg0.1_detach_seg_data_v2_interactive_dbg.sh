...
################## LLaMA-2 ##################
out_dir=/comp_robot/zhanghao/model/llava_stage2_new_joint_seg0.1_detach_seg_data_v2_interactive_dbg
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed --include=localhost:2,3,4,5 --master_port 29113 llava/train/train_hao_joint2st_interactive.py \
    ...
    --max_steps 10000 \
    --config_file_gd \
    configs/openseed/openseed_swint_lang_joint_2st_v2_data_end_with_interaction.yaml \
    --config_file_it \
    configs/semsam/idino_swint_1_part_data_llm_ref_feat_all_16_det_pretrainv1.yaml \
    --opt \
    "detach_seg=True,MODEL.DECODER.WEIGHT_MULTIPLIER=0.1,MODEL.DECODER.COST_CLASS_WEIGHT=4.0,flickr.TEST.BATCH_SIZE_TOTAL=8,flickr.TRAIN.BATCH_SIZE_TOTAL=8,coco_interactive.TRAIN.BATCH_SIZE_TOTAL=8,coco_instruct.TRAIN.BATCH_SIZE_TOTAL=8,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt;MODEL.DECODER.WEIGHT_MULTIPLIER=0.1,coco_interactive.TEST.BATCH_SIZE_TOTAL=8,coco_interactive.TRAIN.BATCH_SIZE_TOTAL=8,MODEL.WEIGHTS=/comp_robot/zhanghao/model/idino_llama_coco_ref_feats_fix_all_16im_det_pretrain/model_0084999.pth"
#    >> $out_dir/log 2>&1
