...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_fixed_vg/
load=/comp_robot/zhanghao/model/llava_stage1_new_gd_flickr_continue1/checkpoint-27000/
mkdir -p $out_dir
echo $out_dir/log
...
#deepspeed --include=localhost:0,1,6,7 --master_port 29103 llava/train/train_hao_seg_flickr.py \
deepspeed  --master_port 29103 llava/train/train_hao_seg_flickr.py \
    --deepspeed scripts/zero2.json \
    --model_name_or_path /comp_robot/zhanghao/ckpts/vicuna/vicuna-7b-v1.3/ \
    --whole_model /comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_fixed/ \
    --load_vision True \
    ...
    --report_to wandb \
    --config_file \
    configs/openseed/openseed_swint_lang_refcoco_vg.yaml \
    --opt \
    vg.TRAIN.BATCH_SIZE_TOTAL=32,COCO.TEST.BATCH_SIZE_TOTAL=32,COCO.TRAIN.BATCH_SIZE_TOTAL=32,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
