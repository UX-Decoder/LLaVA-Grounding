...
################## LLaMA-2 ##################
out_dir=/comp_robot/zhanghao/model/llava_stage2_new_gd_flickr_coco_only_w4_seg0.2
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed llava/train/train_hao_seg_flickr.py \
    ...
    --whole_model /comp_robot/zhanghao/model/llava_stage2_new_gd_flickr_coco_only_w4/checkpoint-2000 \
    --load_vision True \
    ...
    --report_to wandb \
    --config_file \
    configs/openseed/openseed_swint_lang_coco_instruct_coco.yaml \
    --opt \
    MODEL.DECODER.WEIGHT_MULTIPLIER=0.2,MODEL.DECODER.COST_CLASS_WEIGHT=4.0,flickr.TEST.BATCH_SIZE_TOTAL=8,flickr.TRAIN.BATCH_SIZE_TOTAL=8,coco_instruct.TEST.BATCH_SIZE_TOTAL=32,coco_instruct.TRAIN.BATCH_SIZE_TOTAL=32,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
