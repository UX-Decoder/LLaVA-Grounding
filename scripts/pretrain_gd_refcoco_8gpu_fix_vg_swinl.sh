...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_fixed_vg_swinl/
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed  --master_port 29103 llava/train/train_hao_seg_flickr.py \
    ...
    --report_to wandb \
    --config_file \
    configs/openseed/openseed_swinl_lang_refcoco_vg.yaml \
    --opt \
    vg.TRAIN.BATCH_SIZE_TOTAL=48,COCO.TEST.BATCH_SIZE_TOTAL=32,COCO.TRAIN.BATCH_SIZE_TOTAL=16,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_swinl_pano_sota.pt \
    >> $out_dir/log 2>&1
