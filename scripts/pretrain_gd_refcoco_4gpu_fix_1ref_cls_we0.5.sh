...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_fixed_1ref_cls_we0.5/
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed --master_port $port llava/train/train_hao_seg_flickr.py \
    ...
    --report_to wandb \
    --config_file \
    configs/openseed/openseed_swint_lang_refcoco.yaml \
    --opt \
    MODEL.DECODER.COST_CLASS_WEIGHT=0.5,MODEL.DECODER.CLASS_WEIGHT=0.5,MODEL.DECODER.GROUNDING.MAX_LEN=2,flickr.TEST.BATCH_SIZE_TOTAL=32,flickr.TRAIN.BATCH_SIZE_TOTAL=32,COCO.TEST.BATCH_SIZE_TOTAL=32,COCO.TRAIN.BATCH_SIZE_TOTAL=32,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
