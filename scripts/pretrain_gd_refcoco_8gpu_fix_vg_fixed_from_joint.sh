...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/zhanghao/model/llava_stage1_new_gd_refcoco_fixed_vg_fixed_from_joint/
load=/comp_robot/lihongyang/model/llava_stage1_new_flickr_refcoco_llava600k_end_num_fix/checkpoint-18000/
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed  --master_port 29103 llava/train/train_hao_seg_flickr.py \
    ...
    --opt \
    vg.TRAIN.BATCH_SIZE_TOTAL=48,COCO.TEST.BATCH_SIZE_TOTAL=32,COCO.TRAIN.BATCH_SIZE_TOTAL=16,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
