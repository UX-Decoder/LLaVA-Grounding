...
################## LLaMA-2 ##################
out_dir=/comp_robot/zhanghao/model/llava_stage2_new_joint_seg0.1_data_v3_4gpu
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed llava/train/train_hao_joint2st.py \
    ...
    --opt \
    MODEL.DECODER.WEIGHT_MULTIPLIER=0.1,MODEL.DECODER.COST_CLASS_WEIGHT=4.0,flickr.TEST.BATCH_SIZE_TOTAL=4,flickr.TRAIN.BATCH_SIZE_TOTAL=4,coco_instruct.TEST.BATCH_SIZE_TOTAL=32,coco_instruct.TRAIN.BATCH_SIZE_TOTAL=32,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
