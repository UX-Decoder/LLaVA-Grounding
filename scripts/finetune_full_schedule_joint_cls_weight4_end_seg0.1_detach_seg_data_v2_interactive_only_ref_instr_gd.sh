...
################## LLaMA-2 ##################
out_dir=/comp_robot/zhanghao/model/llava_stage2_new_joint_seg0.1_detach_seg_data_v2_interactive_ref_instr_gd
mkdir -p $out_dir
echo $out_dir/log
...
num_gpu=8
bs=$(( 2 * $num_gpu ))
deepspeed llava/train/train_hao_joint2st_interactive_only_ref_instr_gd.py \
    ...
    --max_steps 30000 \
    --config_file_gd \
    configs/openseed/openseed_swint_lang_joint_2st_v2_data_end_with_interaction_only_ref_instr_gd.yaml \
    --config_file_it \
    configs/semsam/idino_swint_1_part_data_llm_ref_feat_all_16_det_pretrainv1.yaml \
    --opt \
    "MODEL.DECODER.WEIGHT_MULTIPLIER=0.1,MODEL.DECODER.COST_CLASS_WEIGHT=4.0,flickr.TEST.BATCH_SIZE_TOTAL=8,flickr.TRAIN.BATCH_SIZE_TOTAL=8,coco_interactive.TRAIN.BATCH_SIZE_TOTAL=${bs},coco_instruct.TRAIN.BATCH_SIZE_TOTAL=32,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt;MODEL.DECODER.WEIGHT_MULTIPLIER=0.2,coco_interactive.TEST.BATCH_SIZE_TOTAL=${bs},coco_interactive.TRAIN.BATCH_SIZE_TOTAL=${bs},MODEL.WEIGHTS=/comp_robot/zhanghao/model/idino_llama_coco_ref_feats_fix_all_16im_det_pretrain/model_0084999.pth" \
    >> $out_dir/log 2>&1
