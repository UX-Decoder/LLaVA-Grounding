...
################## LLaMA-2 ##################
out_dir=/comp_robot/zhanghao/model/llava_stage2_new_joint_seg0.1_detach_seg_data_v2_detail_interleave
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed llava/train/train_hao_joint2st.py \
    --deepspeed scripts/zero2.json \
    --model_name_or_path /comp_robot/zhanghao/ckpts/vicuna/vicuna-7b-v1.3/ \
    --whole_model /comp_robot/zhanghao/model/llava_stage1_new_gd_flickr_continue/checkpoint-27000 \
    --load_vision True \
    ...
    --max_steps 10000 \
    --config_file \
    configs/openseed/openseed_swint_lang_joint_2st_v2_data_detail.yaml \
    --opt \
    detach_seg=True,MODEL.DECODER.WEIGHT_MULTIPLIER=0.1,MODEL.DECODER.COST_CLASS_WEIGHT=4.0,flickr.TEST.BATCH_SIZE_TOTAL=8,flickr.TRAIN.BATCH_SIZE_TOTAL=8,coco_instruct.TEST.BATCH_SIZE_TOTAL=64,coco_instruct.TRAIN.BATCH_SIZE_TOTAL=64,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
