...
PROMPT_VERSION=v1
########### DO NOT CHANGE ###########
out_dir=/comp_robot/zhanghao/model/llava_stage2_new_gd_refcoco_fixed_vg_3_1_4gpu/
mkdir -p $out_dir
echo $out_dir/log
...
deepspeed  --master_port $port llava/train/train_hao_seg_flickr.py \
    ...
    --opt \
    vg.TRAIN.BATCH_SIZE_TOTAL=24,COCO.TEST.BATCH_SIZE_TOTAL=4,COCO.TRAIN.BATCH_SIZE_TOTAL=8,MODEL.WEIGHTS=/comp_robot/zhanghao/ckpts/openseed_o365.pt \
    >> $out_dir/log 2>&1
